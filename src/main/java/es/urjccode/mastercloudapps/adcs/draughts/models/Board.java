...
class Board {
    ...
    private List<Piece> getBetweenDiagonalPieces(Coordinate origin, Coordinate target) {
    ...
    Error isCorrectPairMove(Turn turn, int pair, Coordinate... coordinates) {
    ...
    void removeRandomPieceCanCapture(List<Coordinate> piecesCanCapture, List<Coordinate> removedCoordinates, Coordinate[] coordinates) {
        if(piecesCanCapture.size()>0 && removedCoordinates.size()==0)
        {
            ...
            this.remove(piecesCanCapture.get(random));
        }
    }
